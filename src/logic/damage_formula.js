...
export function calculateFinalDamage(state, gameData) {
    ...
    if (talent.additive_mv_bonus && activeBuffs) {
        ...
        if (sourceBuff?.active && sourceBuff.stacks > 0) {
            ...
            if (bonusInfo.enhancement_buff && activeBuffs[bonusInfo.enhancement_buff]?.active) {
                bonusMultiplier *= (bonusInfo.enhancement_multiplier || 2.0);
            }
    ...
    baseDamage = baseMultiplier * scalingStatValue;

    if (talent.flat_scaling_talent) {
        const flatTalentLevel = characterBuild.talentLevels?.[talent.flat_scaling_talent] || 1;
    ...
    if (activeBuffs) {
        Object.entries(activeBuffs).forEach(([buffKey, buffState]) => {
            ...
            const DYNAMIC_SCALING_STAT_VALUE = totalStats[dynamic.scaling_stat] || 0;
            let finalMultiplier = 0;

            if (dynamic.talent_level_based && dynamic.stack_based_lookup && dynamic.talent && dynamic.values) {
                const holderBuild = characterBuilds[buffDef.source_character];
                const stacks = buffState.stacks || 0;
                if (holderBuild && stacks > 0) {
                    const talentLevel = (holderBuild.talentLevels[dynamic.talent] || 1);
                    const talentLevelKey = String(talentLevel);
                    if (dynamic.values[talentLevelKey]) {
                        finalMultiplier = dynamic.values[talentLevelKey][stacks - 1] || 0;
                    }
                }
            } else if (dynamic.talent_level_based && dynamic.talent && dynamic.values) {
                const holderBuild = characterBuilds[buffDef.source_character];
                if (holderBuild) {
                    const talentLevel = (holderBuild.talentLevels[dynamic.talent] || 1);
                    finalMultiplier = dynamic.values[talentLevel - 1] || 0;
                }
            } else if (dynamic.multiplier) {
                finalMultiplier = dynamic.multiplier;
            }

            let flatBonus = DYNAMIC_SCALING_STAT_VALUE * finalMultiplier;
    ...
    const enemyDefMultiplier = (characterBuild.level + 100) / ((characterBuild.level + 100) + (state.enemy.level + 100) * (1 - defReduction) * (1 - defIgnore));
