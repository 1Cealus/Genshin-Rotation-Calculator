...
import { db, doc, setDoc, onSnapshot, collection, getDocs, deleteDoc, addDoc, Timestamp, updateDoc, where } from '../firebase';
...
export const useAppCore = ({ user, gameData, isUserLoading, isGameDataLoading, isAdmin }) => {
    ...
    const handleSaveToMastersheet = async () => {
     ...
     const submitToAllRelevantLeaderboards = async (uid, userBuilds) => {
        ...
        if (!userBuilds) { 
            ...
            if (profile) {
                userBuilds = profile.characterBuilds;
            } else {
                showModal({ title: "Error", message: "Could not find builds for the specified UID to submit."});
                return;
            }
        }
        try {
    ...
    return {
        page, setPage, showLoginModal, setShowLoginModal, showCreateLeaderboardModal, setShowCreateLeaderboardModal,
        isLoggingIn, isFetchingProfile, logs, setLogs,
        profiles, activeProfile, activeProfileUid,
        setActiveProfileUid, handleProfileLookup, handleCloseProfile,
        team, setTeam, characterBuilds, setCharacterBuilds, enemyKey, setEnemyKey,
        rotation, setRotation, rotationDuration, setRotationDuration,
        presetName, setPresetName, savedPresets,
        mainView, setMainView, activeActionTray, setActiveActionTray,
        editingActionId, setEditingActionId, editingBuildFor, setEditingBuildFor,
        selectedActionIds, setSelectedActionIds, showBulkEdit, setShowBulkEdit,
        importFileRef, currentLeaderboardId, setCurrentLeaderboardId,
        handleTeamChange, updateCharacterBuild, handleAddFromNotation,
        handleAddSingleAction, handleActionRepeatChange, handleDuplicateAction,
        handleRemoveAction, handleUpdateAction, handleActionSelect, handleBulkApplyBuffs,
        onSavePreset: handleSavePreset, onLoadPreset: handleLoadPreset, onDeletePreset: handleDeletePreset,
        onClearAll: handleClearAll, onExport: handleExportData, onImport: handleImportData,
        handleCreateLeaderboard, onSaveToMastersheet: handleSaveToMastersheet, submitToAllRelevantLeaderboards,
        calculationResults, rotationSummary, activeTeam, analyticsData
    };
};